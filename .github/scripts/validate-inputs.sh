...
subnets() { jq -e 'all(.[]; test("^subnet-[0-9a-f]+$"))' <<<"$1" >/dev/null; }
groups() { jq -e 'all(.[]; test("^sg-[0-9a-f]+$"))' <<<"$1" >/dev/null; }

if [[ "$environment" == hml ]]; then
  [[ -n "${HML_PUBLIC_SUBNET_IDS:-}" && -n "${HML_ALLOWED_CIDR_BLOCKS:-}" ]] || { echo 'HML subnet and CIDR inputs are required.' >&2; exit 1; }
  [[ -z "${PROD_PRIVATE_SUBNET_IDS:-}${PROD_ALLOWED_SECURITY_GROUP_IDS:-}${PROD_LAMBDA_SECURITY_GROUP_IDS:-}" ]] || { echo 'PROD inputs are forbidden for HML.' >&2; exit 1; }
  array "$HML_PUBLIC_SUBNET_IDS" && subnets "$HML_PUBLIC_SUBNET_IDS" || { echo 'HML_PUBLIC_SUBNET_IDS must be a JSON array of subnet IDs.' >&2; exit 1; }
  ...
  [[ -n "${PROD_PRIVATE_SUBNET_IDS:-}" ]] || { echo 'PROD private subnet inputs are required.' >&2; exit 1; }
  [[ -z "${HML_PUBLIC_SUBNET_IDS:-}${HML_ALLOWED_CIDR_BLOCKS:-}" ]] || { echo 'HML inputs are forbidden for PROD.' >&2; exit 1; }
  array "$PROD_PRIVATE_SUBNET_IDS" && subnets "$PROD_PRIVATE_SUBNET_IDS" || { echo 'PROD_PRIVATE_SUBNET_IDS must be a JSON array of subnet IDs.' >&2; exit 1; }
